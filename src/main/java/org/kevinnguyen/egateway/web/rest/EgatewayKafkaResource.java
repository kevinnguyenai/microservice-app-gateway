...
@RestController
@RequestMapping("/api/egateway-kafka")
public class EgatewayKafkaResource {

    private final Logger log = LoggerFactory.getLogger(EgatewayKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public EgatewayKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
