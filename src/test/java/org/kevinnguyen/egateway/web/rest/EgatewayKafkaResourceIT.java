...
import org.kevinnguyen.egateway.config.EmbeddedKafka;
import org.kevinnguyen.egateway.config.KafkaSseConsumer;
import org.kevinnguyen.egateway.config.KafkaSseProducer;
...
class EgatewayKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/egateway-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/egateway-kafka/consume")
